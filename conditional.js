...
// Clear values of hidden elements.
...
Drupal.behaviors.conditional = {
  attach: function (context, settings) {
    // AJAX is not updating settings.conditional correctly.
    var conditional = settings.conditional || Drupal.settings.conditional;
    if (typeof conditional === 'undefined' || typeof conditional.effects === 'undefined') {
      return;
    }
    ...
    $.each(eventsData, function(i, events) {
      if (i.substring(0, 6) === 'state:') {
        ...
        events[0].handler = function(e) {
          var effect = conditional.effects[e.target.id];
